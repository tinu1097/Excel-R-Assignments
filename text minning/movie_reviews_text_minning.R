...
library(XML)
king<-NULL
url<-"https://www.imdb.com/title/tt0240772/reviews?ref_=tt_urv"
for(i in 1){
  url<-read_html(as.character(url)) #see. we use this while there is pagination. like page=1,page=2 or anything where reivew are on anohter page.... like we have seen in amazon and snapdeal.. right?yeah and so i tried but there wa no page no mentioned yeah exactly ... its call lazy loading where there is no page actually... see like that...yeah
  ping<-url %>%
    html_nodes(".text") %>%
    html_text() 
  king<-c(king,ping)
