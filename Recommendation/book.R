...
library(reshape2)
book_data <- read.csv("~/Downloads/Recommendation/books.csv",stringsAsFactors = F)
...
#Creating Matrix For Author,Publisher,And Ratings Based.
...
#author_singular_value_decomp = Recommender(binarize(author_matrix_r,minRating=5), method="SVD")
author_popular = Recommender(binarize(author_matrix_r,minRating=5), method="POPULAR")

#Create filter based on publisher.
...
#publisher_singular_value_decomp = Recommender(binarize(publisher_matrix_r,minRating=5), method="SVD")
publisher_popular = Recommender(binarize(publisher_matrix_r,minRating=5), method="POPULAR")

#Create filter based on ratings.
...
#ratings_singular_value_decomp = Recommender(binarize(ratings_matrix_r,minRating=5), method="SVD")
ratings_popular = Recommender(binarize(ratings_matrix_r,minRating=5), method="POPULAR")

#Author based recommendation.
#1,10,333
uid = "2" #Create Recommendation for user 1 based on author.
print("recommendations for you:")
prediction <- predict(author_popular, author_matrix_r[uid],n=5) 
